...
step = 1 
agent = Agent(ALPHA=0.0005, input_dims=14, GAMMA=0.99,
...
while step < 1000:
    ...
    for tl_id in traffic_light_ids:
        ...
        for lane in curr_state[tl_id].controlled_lanes:
            ...
            observation.append((1+curr_state[tl_id].lane_waiting_time[lane])/(1+curr_state[tl_id].lane_vehicle_count[lane]))
            product *= (1+curr_state[tl_id].lane_waiting_time[lane])/(1+curr_state[tl_id].lane_vehicle_count[lane])
        reward =100000000 - ((product)/(average_waititng_time**12))
